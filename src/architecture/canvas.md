...
All "dirty" 2d canvases are stored in `Document` and are flushed on as part of reflow, by sending IPC message for redrawing of each canvas (this corresponds to update the rendering of canvas).
...
flowchart TD
    ...
    UpdateWR@{ shape: hex, label: "presentation id newest" } --yes-->
    Mapped[mapped as GPUPresentationBuffer
    unmap old GPUPresentationBuffer]
...
Both WebRender and `get_image_data` will use content of active mapped presentation buffer ([`GPUPresentationBuffer`](https://github.com/servo/servo/blob/c915bf05fc9abcfba8a64cd4d50166a363a61109/components/webgpu/swapchain.rs#L41)).
