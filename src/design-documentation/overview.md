...
When muliprocess mode is enabled, each script thread runs in its own web content process.
...
There is a single renderer for the entire Servo instance, which manages multiple WebRender instances which render to a veriety of `RenderingContext`s, essentially OpenGL contexts on platform surfaces.
...
* **Selector matching**:
  ...
  Lik Gecko, Servo does selector matching in a separate pass from flow tree construction so that it is more easily parallelized.
* **Parallel layout**:
...
The high-level API will in fact be IPC-based, with non-IPC implementations for testing and single-process use-cases, though it is expected most serious uses would use multiple processes.
